...
class-imbalance handling. Feature representation, imbalance handling and
hyperparameters are selected on a held-out validation slice; final-test
\emph{labels, predictions and performance metrics} are never used for model or
hyperparameter selection (the descriptive exploratory analysis in Section
\ref{sec:eda} runs on the full dataset, including the test period, as is
standard practice for EDA). Bootstrap confidence intervals and a matched-sample
comparison give every model family identical training data. Logistic Regression
with balanced class weights is the
\textbf{strongest observed model under this evaluated setup} (binary macro-F1
$0.888$) without sacrificing feature-level interpretability, including in the
matched-sample comparison;
the headline accuracy of a trivial baseline ($69\%$ on the final test slice) is shown to be misleading
because it never identifies a single negative review. A ternary framing exposes
\textbf{persistent difficulty under the evaluated setup}: the minority 3-star
(neutral) class collapses to below $1\%$ recall for Naive Bayes without
rebalancing, and a dedicated automatic analysis of true 3-star test reviews
shows the model cannot reliably separate mildly-positive from mildly-negative
phrasing. For aspect-based sentiment (RQ2) we segment
reviews into sentences, detect eight aspects with a cleaned keyword lexicon,
use noun phrases for exploratory candidate discovery, and score each
aspect-mentioning sentence with VADER --- a
lexicon scorer chosen because the document classifier is out-of-distribution on
single sentences. The lexicon deliberately excludes broad product identifiers
such as \emph{box}, \emph{magazine} and \emph{subscription}, and removes sentiment
words from aspect detection. Across both categories, \emph{billing}
(cancel/refund/charged) is the \textbf{weakest high-coverage aspect (mention rate
$\geq 5\%$) in both categories}, while \emph{content}, \emph{quality} and
\emph{price} are the most positive high-coverage aspects. Billing-related
complaint language is frequent and scores negative under the lexicon,
especially for Subscription Boxes, and a clause-level sensitivity analysis
reinforces the direction of this signal; however, a billing-lexicon masking
check shows the polarity is \textbf{not robust for Subscription Boxes}: masking
the triggering terms themselves shifts the mean sentiment from $-0.080$ to
$+0.044$, reversing its sign, so part of the unmasked negativity reflects the
lexicon valence of words like \emph{cancel} rather than surrounding review
context alone. The strongest domain-specific negative coefficient of the
interpretable classifier is the token \emph{cancel} --- complementary, not
independently confirmatory, evidence for the same billing signal the aspect
pipeline surfaces. All findings are descriptive and associative, not causal.
...
above all, \emph{interpretable} --- here meaning coefficient-based, global
...
non-parametric bootstrap confidence intervals throughout RQ1 and RQ2.
...
model that can answer ``\emph{which words drove this prediction?}'' is high,
...
chronological holdout that is leakage-free \emph{within each category}
...
sentiment on the \emph{same} corpus (including a clause-level sensitivity
check); and two small, under-studied subscription categories rather than a
large, well-trodden one.
...
timestamp and split \emph{within each category} so both categories appear on
every slice, preserving the cross-category comparison RQ2 needs. Chronological
order is therefore preserved \emph{within each category}; because the
per-category splits are then combined into one dev-train/validation/test set
...
reviews in the combined test set). The leakage-free, temporal guarantee this
split provides is therefore \emph{within-category}, not global: no review
...
A plain oldest-80\%/newest-20\% split is not enough on its own: if the held-out
...
\textbf{64\% (dev-train)}, next \textbf{16\% (validation)}, and newest
\textbf{20\% (final test)}. Feature representation (unigram vs.\ uni+bigram),
...
\text{balanced}\}$ --- are all selected using \emph{only} the validation slice,
...
80\% (dev-train + validation) and evaluated on the final 20\%. Final-test
\emph{labels, predictions and performance metrics} are never used for model,
...
decisions above, was not shielded from the test period --- acceptable for
...
\textbf{\texttt{sklearn.ensemble.GradientBoostingClassifier}}
...
comparison would conflate \emph{model-family capability} with
\emph{training-data volume}. We therefore report two distinct blocks on the
final test set: a \textbf{matched-sample comparison}, in which all three
families train on an identical, deterministically sampled 12,000-row training
set, and a \textbf{full-data comparison} (Naive Bayes and Logistic Regression
...
regime (full dev-train for Naive Bayes and Logistic Regression, the internal
...
reported with non-parametric bootstrap 95\% confidence intervals (2,000
replications, fixed seed, percentile method) following \citet{efron1993bootstrap},
stratified by true class for per-class recall. The unigram-vs-bigram comparison
uses a \emph{paired} bootstrap (same resampled indices for both feature
...
eight aspects --- \emph{delivery, packaging, quality, price, content, customer
service, billing} and \emph{advertising}. The last three are domain-specific:
...
\emph{box}, \emph{magazine} and \emph{subscription}. A complementary, lexicon-free
...
fixed rule. We define an aspect as \textbf{high-coverage within a category iff
...
Sentence-level attribution cannot separate two aspects that co-occur in one
...
of \emph{but, however, although, though, yet, while, whereas}, and attribute
each aspect only to the clause(s) that contain its keyword (Section
\ref{sec:rq2-clause}).

\paragraph{Billing-lexicon sensitivity check.}
The billing keyword list contains words such as \emph{cancel}, \emph{refund}
and \emph{charged} that often already occur in problem-oriented contexts. We
...
is a leakage/lexicon-sensitivity check, not an alternative primary method.
...
therefore resample whole \emph{reviews}, not individual rows, for bootstrap 95\%
...
a held-out \textbf{validation} slice (16\% of the data) and never on the
...
Naive Bayes and Gradient Boosting configuration tried, \emph{before} the final
...
illustrates \emph{why} imbalance handling matters,
using the validation slice (not the final test set, and not used for any
selection decision). In both framings the majority baseline has \textbf{zero}
...
Binary & \textbf{Log.\ Reg.\ (balanced)} & \textbf{0.918} & \textbf{0.888} & 0.926 & -- & 0.889 \\
\midrule
Ternary & Majority baseline          & 0.719 & 0.279 & 1.000 & 0.000 & 0.000 \\
Ternary & \textbf{Log.\ Reg.\ (balanced)} & \textbf{0.813} & \textbf{0.651} & 0.880 & 0.449 & 0.712 \\
\bottomrule
\end{tabular}
\end{table}

On the final test set, accessed only after selection, the chosen binary
configuration reaches accuracy $0.902$, macro-F1 $\mathbf{0.888}$,
...
$0.785$, macro-F1 $\mathbf{0.644}$, recall$_{\text{pos}}=0.848$,
...
shows \textbf{persistent difficulty under the evaluated setup} on every
held-out slice we tried (not a provable, universal ceiling) because such
reviews genuinely mix positive and negative content --- directly motivating
...
For the unigram-vs-bigram comparison, both configurations were retrained on
the full oldest 80\% and evaluated on the final test set with a
\emph{paired}, equally stratified bootstrap (same resampled indices for
...
this as a \textbf{stable, if modest, difference under this holdout} --- not a
...
final test set: a \textbf{matched-sample} comparison (all three families on an
identical, deterministically sampled 12,000-row training set) and a
\textbf{full-data} comparison (Naive Bayes and Logistic Regression only ---
...
Matched (A) & Logistic Regression   & 12{,}000 & 0.891 & \textbf{0.876} & 0.858 \\
Matched (A) & \texttt{GradientBoostingClassifier} & 12{,}000 & 0.841 & 0.815 & 0.750 \\
\midrule
Full data (B) & Naive Bayes         & 25{,}230 & 0.870 & 0.857 & 0.909 \\
Full data (B) & Logistic Regression & 64{,}456 & 0.902 & \textbf{0.888} & 0.884 \\
\bottomrule
\end{tabular}
\end{table}

Logistic Regression wins \textbf{even in the matched-sample block}, where
...
Block B, this supports calling Logistic Regression the \textbf{strongest
...
features (e.g.\ \emph{not worth}, \emph{absolutely not}) survive preprocessing
...
terms such as \emph{disappointed, cancel, disappointing, poor} and the
bigram \emph{cancel subscription}. The strongest overall negative coefficient is
\emph{not}, which confirms that preserving negation matters. Among the
domain-specific terms, \emph{cancel} is the strongest billing signal and the
...
a \textbf{rule-based exploratory categorisation} (Appendix Table
\ref{tab:err}) --- simple keyword/length heuristics written by the author, not a
...
($47$ of $100$), followed by very short/context-free reviews ($25$) and mixed
...
\textbf{reproducible, automatic exploratory analysis} (contrastive-marker and
...
($0.61$--$0.63$) and barely differs between correct and incorrect predictions,
...
(\emph{but/however/although/though/yet/while/whereas}) appears in $41.6\%$ of
...
gets predicted neutral correctly. This directly answers the RQ1 sub-question on
...
Table \ref{tab:rq2}). After cleaning the lexicon, \emph{content} is the largest
aspect in both categories: $43.0\%$ of Subscription-Box reviews and $38.5\%$ of
Magazine reviews mention product or editorial substance. \emph{Price} and
\emph{delivery} are secondary themes for boxes, while magazines show more
\emph{price} and \emph{advertising} discussion. Crucially, \emph{packaging} now
appears in only $6.1\%$ of box reviews; the earlier high value was a product-name
artefact caused by matching \emph{box/boxes}, not evidence that packaging
...
Table \ref{tab:rq2} agree on one pattern: \emph{billing} is high-coverage
(mention rate $\geq 5\%$, Section \ref{sec:method-coverage}) in \emph{both}
categories and is the \textbf{weakest high-coverage aspect in both} (compound
...
\emph{not} describe billing there as an unconditionally negative pain point ---
only as the weakest aspect among the high-coverage ones. This is much sharper than the
earlier broad-\emph{subscription} version of the lexicon: once neutral product
mentions are removed, billing's negative skew for boxes becomes visible rather
than being diluted by neutral mentions. \emph{Content} and \emph{quality} are
the most positive high-coverage aspects in boxes (compound $\approx
0.33$--$0.34$), while magazines are most positive about \emph{content} and
\emph{price} (compound $\approx 0.32$--$0.33$). Advertising is high-coverage
...
VADER--classifier agreement subsample) it is below the high-coverage threshold
and is treated as a low-coverage signal, not a primary finding.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{rq2_aspect_radar.png}
\caption{Aspect profiles as radar charts, with a dynamic, non-clipping radial
...
billing          & 9.3  & \textbf{-0.091} & 56.9 & \checkmark & 8.5  & \textbf{0.051} & 33.4 & \checkmark \\
...
are product/category nouns --- \emph{the box}, \emph{this magazine},
\emph{the subscription} --- and are intentionally \emph{not} mapped to an aspect,
because doing so would recreate the artefact the cleaned lexicon removes. As a
simple keyword-vs.-noun-phrase check, $8$ of the top $30$ box phrases and $10$ of
the top $30$ magazine phrases map back to at least one lexicon aspect. Matched
phrases include \emph{the toys}, \emph{the items}, \emph{the price}, \emph{ads},
\emph{articles} and \emph{recipes}; the unmatched remainder is mostly generic
...
configuration, Section \ref{sec:method-split}) agree on $77.1\%$ of
non-neutral aspect mentions ($n=789$; Appendix Table \ref{tab:agree}). VADER labels
$26.1\%$ of sampled aspect mentions as \emph{neutral}, a category the binary
...
\textbf{This is not an independent or blinded manual annotation}: the
...
reported only as an \textbf{illustrative qualitative spot-check}, not as a
...
\emph{box} no longer creates packaging hits, \emph{subscription} no longer
...
whether it drives the headline findings by re-scoring on a conservative
clause split (a semicolon, or one of \emph{but}, \emph{however},
\emph{although}, \emph{though}, \emph{yet}, \emph{while}, \emph{whereas}),
...
\emph{ranking} is essentially unchanged: the rank correlation between
...
\emph{strengthening}, not weakening, the billing finding. Among reviews with at
least one detected aspect, $17$--$22\%$ have at least one aspect whose sentiment
...
replacement right away.'' At sentence level, \emph{content} (triggered by
\emph{item}) inherits the whole second sentence's positive compound ($0.812$,
dominated by ``super nice'', ``replacement right away''); at clause level,
\emph{content} is correctly scored from its own clause alone ($-0.649$),
while \emph{customer\_service} keeps the positive clause. The main RQ2
...
\emph{cancel}/\emph{cancelled} score $-0.250$, \emph{charged} scores $-0.202$,
and \emph{charges} scores $-0.273$, while most others (\emph{refund},
\emph{billing}, \emph{renewal}, \ldots) score $0.0$ alone. We therefore recompute billing
...
notably toward positive in \emph{both} categories: Magazine Subscriptions
$0.036\rightarrow0.085$ ($+0.049$); Subscription Boxes
$-0.080\rightarrow+0.044$ ($+0.124$, \textbf{reversing the sign}). This is an
...
words like \emph{cancel} and \emph{charged} themselves, not solely the
surrounding review context. We do \emph{not} adopt the masked variant as a new
primary method --- masking also strips genuine sentiment-bearing context in
many sentences (e.g.\ ``terrible --- they keep charging me after I
cancelled'', where \emph{terrible} and the surrounding clause carry sentiment
...
resample whole \emph{reviews} (not rows) for 95\% bootstrap CIs of mean aspect
...
--- excluding zero, so we describe a stable cross-category difference under
...
includes zero, so we do \emph{not} claim a stable difference there.
...
Logistic Regression is the \textbf{strongest observed model under the
...
Gradient Boosting in \emph{both} the matched-sample comparison ($0.876$ vs.\
...
negation cue \emph{not}; among domain-specific features, \emph{cancel} is the
strongest and the fifth-largest negative coefficient overall. The
sentence-level VADER pipeline also identifies \emph{billing} as the weakest
high-coverage aspect in both categories. These two analyses provide
\textbf{complementary, triangulating evidence} for billing-related
dissatisfaction as a recurring subscription-product signal, rather than one
independently confirming the other --- both ultimately rely on overlapping
...
flags \emph{which} facet drives sentiment. Billing is the \textbf{weakest
...
polarity is \textbf{not robust for Subscription Boxes} once the triggering
...
\ref{sec:limitations}), these recommendations need re-validation before
extending to non-subscription Amazon categories.
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Sentence-level attribution} is the primary method and cannot,
  by itself, separate two aspects that co-occur in one sentence with mixed
  ...
  partial, not exhaustive, probe of this concern. The \emph{content} lexicon
  in particular includes broad terms (\emph{item, product, issue}) so that it
  can match generic mentions of what is being reviewed; for Magazine
  Subscriptions \emph{issue} is ambiguous between ``magazine issue'' and
  ...
  second method and an \emph{illustrative}, self-conducted qualitative
  ...
  \item \textbf{Star ratings as labels} are noisy, as the exploratory error
  ...
  \item \textbf{Hyperparameter and feature search} used a small, deterministic
  ...
  \item \textbf{Cross-category scope:} both categories studied are subscription
  ...
  \item \textbf{Associative, not causal:} all RQ1 and RQ2 findings, including
...
with an interpretable balanced Logistic Regression the \textbf{strongest
observed model} ($0.888$ macro-F1, final test) under the evaluated setup ---
...
review-level bootstrap CIs) shows that \emph{what} customers discuss is mostly
content, while \emph{what frustrates} them is shared and concentrated in
\emph{billing} --- the weakest high-coverage aspect in both categories.
...
triggering terms (\emph{cancel}, \emph{refund}, \emph{charged}) themselves
...
\emph{cancel} --- the same billing signal --- means the two analyses provide
...
clause-level/billing-lexicon sensitivity, and top-noun-phrase tables; and the
...
\begin{table}[H]
\centering
\caption{Agreement between VADER and the document classifier on non-neutral aspect mentions (test sample).}
\label{tab:agree}
\begin{tabular}{lrr}
\toprule
Aspect & Agreement & $n$ \\
...
\noindent
\theplace, \thedate \hfill \rule{6cm}{0.4pt}\\
\phantom{\theplace, \thedate} \hfill \theauthor
