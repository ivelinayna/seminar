...
continuous, survey free feedback signal, provided the text can be read at scale.
...
labels, predictions and performance metrics are never used for model or
...
with balanced class weights is the strongest observed model under this
evaluated setup (binary macro-F1 $0.888$) without sacrificing feature level
interpretability, including in the matched sample comparison;
...
persistent difficulty under the evaluated setup: the minority 3 star
...
such as box, magazine and subscription, and removes sentiment
words from aspect detection. Across both categories, billing
(cancel/refund/charged) is the weakest high coverage aspect (mention rate
$\geq 5\%$) in both categories, while content, quality and
price are the most positive high coverage aspects. Billing related
...
check shows the polarity is not robust for Subscription Boxes: masking
...
lexicon valence of words like cancel rather than surrounding review
context alone. The strongest domain specific negative coefficient of the
interpretable classifier is the token cancel, complementary, not
